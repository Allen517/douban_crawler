...
class UserRelsCrawler(object):
	...
	def __getDoubanUserRelsOnePage(self, uid, s):
		...
		url = "https://api.douban.com/shuo/v2/users/{user}/following?count=50&start={start}&apikey=09ec337ecc4d17531dc49a7c64e745f9".format(user=uid, start=s)
	...
	def __getDoubanUserRels(self, uid):
		...
		sleep(self.sleep_time) #休眠25秒后继续抓取
		if not rels:
			self.fail_count += 1
	...
	def getDoubanUserRels(self):
		...
		while uru_index<len_uru:
			...
			if u_index>=0:
				...
				if not rels and self.retry_num<self.max_retry:
					self.db_user.updateUserRelsCrawlTag(uid, 1)
